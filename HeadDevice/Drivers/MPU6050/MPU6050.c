...
HAL_StatusTypeDef GetRawAcc(struct MPU6050_ACCResult *result) 
{
	...
	HAL_StatusTypeDef statResult = I2C_BufferRead(MPU6050_DEFAULT_ADDRESS,
			I2C1_Buffer_Rx, MPU6050_RA_ACCEL_XOUT_H, 6);
...
HAL_StatusTypeDef GetRawGyro(struct MPU6050_GYROResult *result) {
...
void try_start_mpu(UART_HandleTypeDef *huart, I2C_HandleTypeDef *hi2c){
	...
	#include <string.h>
	HAL_StatusTypeDef status = Initialize();
	while(status != HAL_OK) {
		myError err = I2CErrorHandling(&hi2c1, status);
		err = Wrap(err, "I2CErrorHandling: mpu doesnt connect: "); 
		HAL_UART_Transmit(huart, (uint8_t*)err.msg.msg, err.msg.size, HAL_MAX_DELAY);
		HAL_Delay(300);
		}       
}
